...
- `litertlm_manifest` — the repo ships `litertlm_manifest.json`; the generator cross-checks each file's sha256 / size against it, refuses a profile whose language backend the manifest does not list as verified, imports `context_length`, and refuses a bundle that declares a thinking channel unless `handler_config.channels` spells it out;
