...
| `NO_COMPATIBLE_PROFILE` | No profile fits the device, the policy or `requiredInputs`; `details` lists each profile and why. | Loosen the policy (`Auto`), or pick a variant that has a fitting profile. |
