...
| `CONTEXT_LIMIT_EXCEEDED` | The prompt plus history exceed the context. On a decision model: a question's options do not fit the head of the window (`details["question"]`); a long state is cut and reported as `truncated`, never an error. | Start a new conversation or shorten the input. Decision model: fewer or shorter options, or a variant with a larger window. |
...
| `INITIALIZATION_FAILED` | `Engine.initialize()` / `createConversation` failed; `details["stage"]` and `fallback_tried` say where. On a decision model: the tokenizer files did not load (`stage=tokenizer`) or `CompiledModel` failed to compile the graph on the profile's accelerator (`stage=compile`; a float16-weight graph on the GPU does this on the measured phone). | Try another profile (`Require(CPU)`); check free RAM; keep the message for the publisher. Decision model: `Auto` falls back to the profile's `fallback_profiles`; a variant with fp32 weights compiles on the GPU. |
